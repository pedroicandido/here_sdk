...
public class SwiftHereSdkBasePlugin: NSObject {
    ...
    public static func register(instance: SwiftHereSdkBasePlugin, registrar: FlutterPluginRegistrar) {
        ...
        } catch SDKInitializer.SDKInitError.missingPlistFile,
                SDKInitializer.SDKInitError.missingCredentialsSection {
            let credentialsNotFound =
                "Warning: Credentials were not found in plist. " +
                "It might happen in the following cases: " +
                "1. You are calling SDKNativeEngine.makeSharedInstance() in Flutter: " +
                "In this case, just ignore this error. " +
                "2. Credentials are missing and you should " +
                "specify your credentials in the section `HERECredentials` in plist. " +
                "Note that this approach is deprecated and it will be removed with HERE SDK 4.15.0. " +
                "Please refer to the developer's guide for more information."

            os_log("%@", type: OSLogType.error, credentialsNotFound)
        } catch  {
