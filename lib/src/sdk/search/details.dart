...
class Details {
  ...
  /// autosuggest/browse/lookup/discover queries are enhanced with truck attributes.
  /// The type string values are one of : hasParking, hasSecureParking, hasIdleReductionSystem,
  /// hasTruckScales, hasTruckService, hasTruckStop, hasTruckWash, showerCount, hasHighCanopy,
  /// hasCarWash, hasWifi, hasPowerSupply, hasChemicalToiletDisposal.
  /// Currently this field has value only for online search.
  /// Note: This is a BETA feature and thus subject to change.
  /// Can be enabled by using `SearchEngine.set_custom_option()` with next arguments:
  ...
  /// This is a online-search-only feature so it is always null with offline search.
  FuelStation? fuelStation;

  Details(List<Contact> contacts, List<OpeningHours> openingHours, List<PlaceCategory> categories, List<WebImage> images, List<WebEditorial> editorials, List<WebRating> ratings, List<SupplierReference> references, [EVChargingPool? evChargingPool = null, TruckAmenities? truckAmenities = null, FuelStation? fuelStation = null])
    : contacts = contacts, openingHours = openingHours, categories = categories, images = images, editorials = editorials, ratings = ratings, references = references, evChargingPool = evChargingPool, truckAmenities = truckAmenities, fuelStation = fuelStation;
  Details.withDefaults(this.contacts, this.openingHours, this.categories, this.images, this.editorials, this.ratings, this.references)
      : evChargingPool = null, truckAmenities = null, fuelStation = null;
...
Pointer<Void> sdkSearchDetailsToFfi(Details value) {
  ...
  final _result = _sdkSearchDetailsCreateHandle(_contactsHandle, _openingHoursHandle, _categoriesHandle, _imagesHandle, _editorialsHandle, _ratingsHandle, _referencesHandle, _evChargingPoolHandle, _truckAmenitiesHandle, _fuelStationHandle);
